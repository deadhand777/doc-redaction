...
#
#     # Emails
#     results["email_addresses"] = list(set(EMAIL_RE.findall(text)))
#
#     # Phones
#     phone_numbers = {
#         match for regex in PHONE_REGEXES for match in regex.findall(text) if len(re.sub(r"[^\d]", "", match)) >= 7
#     }
#     if phone_numbers:
#         results["phone_numbers"] = list(phone_numbers)
#
#     # Credit cards
#     results["credit_card_numbers"] = list({
#         cc for cc in CC_RE.findall(text) if 13 <= len(re.sub(r"[^\d]", "", cc)) <= 19
#     })
#
#     # IBAN
#     iban = IBAN_RE.findall(text)
#     if iban:
#         results["iban_numbers"] = list(set(iban))
#
#     # Accounts
#     accounts = ACCOUNT_RE.findall(text)
#     if accounts:
#         results["account_numbers"] = list(set(accounts))
#
#     # Addresses
#     addresses = {match for regex in ADDRESS_REGEXES for match in regex.findall(text)}
#     if addresses:
#         results["addresses"] = list(addresses)
#
#     # Names
#     names = [n for n in NAME_RE.findall(text) if n not in COMMON_NON_NAMES]
#     if names:
#         results["people_names"] = list(set(names))
#
#     # Currency
#     currency = {match for regex in CURRENCY_REGEXES for match in regex.findall(text)}
#     if currency:
#         results["currency_amounts"] = list(currency)
#
#     # Percentages
#     percentages = {match for regex in PERCENTAGE_REGEXES for match in regex.findall(text)}
#     if percentages:
#         results["percentages"] = list(percentages)
#
#     # Numbers (digits + German words + English words)
#     numbers = {match for regex in NUMBER_REGEXES for match in regex.findall(text)}
